...
summary: "Ace your next Javascript Interview - By Yaqoob"
...
if (length > 5) {
  let welcome = "Hi Yaqoob, welcome to block scope";
  console.log(welcome);
}
...
```js
localStorage.setItem("name", "Yaqoob"); // set an item
...
Here, "Yaqoob" is logged after 2 seconds.
...
Here, "Yaqoob" is printed every 2 seconds.
...
```js
function start() {
  var name = "Yaqoob"; // name is a local variable created by start()
...
start(); // "Yaqoob" alert box is displayed
```
...
Here,
The instance of displayName maintains a reference to its lexical environment, within which the variable name exists. For this reason, when fun() is invoked, the variable name remains available for use, and "Yaqoob" is passed to alert.
...
[Yaqoob create username debounce
...
Don't forget to look at the [Resources](https://yaqoobhalepoto.engineer/resources) and [Snippets](https://yaqoobhalepoto.engineer/snippets) page where I regularly update cool stuff for developers to make their lives easier.
