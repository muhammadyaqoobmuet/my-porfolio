...
const TimelineItem = ({ title, description, icon: Icon }) => (
  <div className="flex gap-4 mb-8 relative">
    ...
    <div className="flex flex-col items-center top-1.5 relative">
        ...
        <div className="w-px h-full bg-gray-200 dark:bg-zinc-800 absolute top-3 -bottom-8" />
    </div>

    <div className="flex-1 pb-2">
        <div className="flex items-center gap-2 mb-2">
            <h4 className="font-sans font-medium text-gray-900 dark:text-zinc-100 text-lg">
                {title}
            </h4>
            {Icon && <Icon className="w-4 h-4 text-gray-500 dark:text-zinc-500" />}
        </div>
        <p className="text-gray-600 dark:text-zinc-400 text-sm leading-relaxed max-w-2xl">
            {description}
        </p>
    </div>
  </div>
...
const Section = ({ year, items }) => (
  <div className="mb-12 last:mb-0">
    <h3 className="font-serif text-xl font-bold text-gray-900 dark:text-white mb-6 pl-0">
        {year}
    </h3>
    <div className="pl-2">
        {items.map((item, idx) => (
...
const JourneyList = () => {
  ...
  return (
    <div className="w-full py-20 max-w-3xl">
      <h2 className="font-serif text-3xl font-bold mb-16 text-gray-900 dark:text-zinc-100">
      ...
      <button
        ...
        className="flex items-center gap-2 text-sm font-medium text-gray-500 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-zinc-300 transition-colors mt-8 mx-auto"
      >
